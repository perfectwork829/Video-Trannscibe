<template>
  <div class="video-player-final-wrapper">
    <div class="video-wrapper" ref="init_video_container">
      <video
        class="video_player_final" 
        ref="init_video"
        :controls="false"
        :autoplay="false"
        @play="(e) => videoResume(e, 0)"
        @pause="(e) => videoPause(e, 0)"
        @timeupdate="(e) => videoTimeUpdate(e, 0)"
        @seeked="(e) => videoSeeked(e, 0)"
        @volumechange="onVolumeChange"
       >
      </video>
    </div>
    <div class="video-wrapper" ref="video_player_final" 
      v-for="(item, index) of sentences.filter((item, i) => i > 0)" :key="index"
      style="display: none">
      <video
        class="video_player_final" 
        ref="video"
        :controls="false"
        :autoplay="false"
        @play="(e) => videoResume(e, index + 1)"
        @pause="(e) => videoPause(e, index + 1)"
        @timeupdate="(e) => videoTimeUpdate(e, index + 1)"
        @seeked="(e) => videoSeeked(e, index + 1)"
        @volumechange="onVolumeChange"
       >
      </video>
    </div>
    <div class="video-ctrl-btn-wrapper mt-3">
      ...
      <b-button class="mr-1" variant="primary" @keyup="e=> e.preventDefault()" @click="onPause" v-else>
...
export default {
  ...
  mounted() {
    ...
    // this.player = new this.shaka.Player(this.$refs.video);
    // const ui = new this.shaka.ui.Overlay(
    //   this.player,
    //   this.$refs.video_player_final,
    //   this.$refs.video
    // )
    // ui.getControls();
  },
  computed: {
    ...mapGetters(['transcriptInfo','paperInfo', 'isPaperPlaying', 'paperPlayerVol']),
    clipsArray: function () {
      return [...this.paperInfo.clips];
    },
    papers: function() {
      return [...this.paperInfo.paperData.map(it => {
        return {
          label: it.title,
          value: it.id
        }
  ...
  watch: {
    clipsArray: {
      ...
      handler() {
        ...
        const curPaper = this.paperInfo.paperData.filter(it => it.id === this.paperInfo.curPaperId)[0];
        this.paperTitle = curPaper ? curPaper.title :'';
        // this.papers = this.paperInfo.paperData.map(it => {
        //   return {
        //     label: it.title,
        //     value: it.title
        //   }
        // })
        this.sentences = [];
        // this.$refs.init_video_container.style.display = 'flex';
        this.curClipIndex = 0;
  ...
  methods: {
    ...
    loadInitVideo: function (item) {
      const player = new this.shaka.Player(this.$refs.init_video);
      const ui = new this.shaka.ui.Overlay(
        player,
        this.$refs.init_video_container,
        this.$refs.init_video
      )
      ui.configure({
      ...
      // ui.getControls();
        ...
        .then(() => {
          this.$refs.init_video.currentTime = item.startTime;
          this.$refs.init_video.play()
          this.$refs.init_video.volume = this.paperPlayerVol
        })
    ...
    onPlay: function () {
      console.log('play')
      // const videoElement = this.$refs.video_player_final;
      ...
      if (this.isPlayFinished) {
        ...
        if (this.sentences.length) {
          this.loadInitVideo(this.sentences[0]); 
          this.curClipIndex = 0;
        }
        this.isPlayFinished = false;
        setTimeout(() => {
          this.sentences.forEach((sentence, index) => {
            if (index > 0) this.loadNewVideo(sentence, index-1)
          })
        }, 100)
      } else {
        console.log('here');
        if (this.curClipIndex === 0) {
          this.$refs.init_video.play();
          this.$refs.init_video.volume = this.paperPlayerVol;
        } else {
          this.$refs.video[this.curClipIndex - 1].play();
          this.$refs.video[this.curClipIndex - 1].volume = this.paperPlayerVol;
        }
      }
    },
      ...
      if (this.sentences[index] == 'interval') {
        ...
        if (e.target.currentTime >= this.sentences[index].endTime) {
            ...
            if ( this.sentences.length > 1 && index < this.sentences.length - 1) {
              ...
              if ( index === 0) {
                this.$refs.init_video_container.style.display = 'none';
                this.$refs.video_player_final[index].style.display = 'flex';
              } else {
                this.$refs.video_player_final[index-1].style.display = 'none';
                this.$refs.video_player_final[index].style.display = 'flex';
              }
              this.$refs.video[index].currentTime = this.sentences[index+1].startTime;
    ...
    onPause: function () {
      ...
      if (this.curClipIndex === 0) {
        this.$refs.init_video.pause();
      } else {
        this.$refs.video[this.curClipIndex - 1].pause();
      }
      this.videoButtonVisible = true;
    },
    onStop: function () {
      ...
      if (this.curClipIndex === 0) {
        this.$refs.init_video.pause();
        this.$refs.init_video.currentTime = 0;
      } else {
        this.$refs.video[this.curClipIndex - 1].pause();
        this.$refs.video[this.curClipIndex - 1].currentTime = 0;
      }
      this.videoButtonVisible = true;
      this.isPlayFinished = true;
      this.curClipIndex = 0;
    ...
    onVolumeChange: function(event) {
      console.log(';test', event.target.volume)
